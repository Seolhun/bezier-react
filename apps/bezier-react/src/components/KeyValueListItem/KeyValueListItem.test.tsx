  ...
  describe('Props', () => {
    describe('keyIcon', () => {
      it('keyIcon이 Bezier의 아이콘 한 종류면, icon으로 렌더링된다.', () => {
        ...
        const keyItemIconWrapper = rendered?.firstChild
        const keyItemIcon = keyItemIconWrapper?.firstChild
    ...
    describe('keyContent', () => {
      it('keyContent가 string이면, Key의 text string으로서 렌더링된다.', () => {
        const { getByTestId } = renderComponent()
        const rendered = getByTestId(TEST_ID_MAP.KEY_ITEM)
        const keyItemTextWrapper = rendered?.lastChild
        const keyItemText = keyItemTextWrapper?.firstChild
  ...
  describe('Styles', () => {
    describe('KeyValueListItem', () => {
      ...
      it('KeyValueListItem > KeyItem', () => {
        ...
        expect(rendered).toHaveStyle('gap: 8px;')
